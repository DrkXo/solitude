...
class EbookLibraryService extends BaseService {
  ...
  final List<String> _failedEbooks = [];
  ...
  List<String> get failedEbooks => List.from(_failedEbooks);
  ...
  /// Throws [EbookLibraryException] if the file is invalid or already exists.
  ...
  /// Throws [EbookLibraryException] if the ebook is not found.
  Future<void> removeEbook(String id) async {
    await executeDBOperation(
      () => _localDbService.solitude.ebookDao.removeEbook(id),
    );
    final index = _ebooks.indexWhere((entry) => entry.id == id);
    if (index == -1) {
      throw EbookLibraryException('Ebook with id $id not found');
    }
    _removeFromEbooksList(index);
  }
  ...
  /// Throws [EbookLibraryException] if the ebook is not found.
  Future<void> updateEbook(EbookEntry updatedEntry) async {
    await _updateEbookInDb(updatedEntry);
    final index = _ebooks.indexWhere((entry) => entry.id == updatedEntry.id);
    if (index == -1) {
      throw EbookLibraryException('Ebook with id ${updatedEntry.id} not found');
    }
    _updateEbooksList(updatedEntry, index: index);
  }

  Future<void> _updateEbookInDb(EbookEntry entry) async {
    await executeDBOperation(
      () => _localDbService.solitude.ebookDao.updateEbook(
        DbEbooksCompanion(
          ...
          currentChapter: Value(entry.currentChapter),
  ...
  void _updateEbooksList(EbookEntry entry, {int? index}) {
    if (index != null) {
      _ebooks[index] = entry;
    } else {
      _ebooks.add(entry);
  ...
  void _removeFromEbooksList(int index) {
    _ebooks.removeAt(index);
    _ebooksController.add(List.from(_ebooks));
  }
}
